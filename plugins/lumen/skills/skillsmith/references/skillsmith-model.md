...
Before inspecting or creating files, determine whether the requested capability is clear enough that a plausible but wrong skill can be distinguished from the intended one. Reuse the current request, active conversation, attached sources, and any host-provided prior context before asking the user again. Repository contents, loaded skills, plugin inventory, and nearby artifacts may constrain implementation after the outcome is known, but they cannot supply a missing user intention. The current instruction always takes precedence.
...
When this boundary changes, treat it as governing. Trace the corrected relation through readiness signals, state transitions, context eligibility, task-model construction, validation artifacts, deterministic checks, mutation fixtures, generated packages, and fresh behavior tests. Remove any older path that lets generated grounding certify generated output.
...
- the current files, task material, host-provided prior context, or direct user evidence that can support each signal;
- context ownership and permitted use for the current run;
...
```text
current task context -> capability calibration or executable state -> task model -> artifact creation -> direct inspection -> Lumen-first repair loop -> independent cold reader -> Lumen again with reader evidence -> fresh final cold reader on the identical version -> user acceptance or correction
```
...
The post-creation sequence is a fixed-point loop, not a checklist whose approvals survive editing. Lumen clears the exact candidate before an acceptance cold reader sees it. The independent reader then applies the capability-specific threshold to that same version. Any repair invalidates both results and returns the candidate to Lumen. After the reader passes, Lumen sees the unchanged artifact again together with the reader evidence, and a fresh final reader evaluates the exact Lumen-cleared version. Release occurs only when both systems pass one identical artifact hash. “Perfect” means every defined hard gate reaches its stated threshold; it does not mean an average score or limitless polishing.

Keep that reader genuinely cold. Use a new isolated subagent or session rather than the author, a follow-up turn, or a reviewer that saw earlier attempts. Give it the exact encounter surface and only the minimum audience, task, or decision context. Do not reveal the intended reading, suspected failure, prior feedback, rejected versions, or a rubric that supplies the answer. Record the reader's unprompted reconstruction and first hesitation before applying capability-specific gates. A favorable score cannot erase a material misunderstanding. Any repair requires a new artifact hash and a new reader.
...
## Keep Learning Inside The Active Task

Use the active conversation and any host-provided prior context to avoid repeated setup during the current run. Keep confirmed corrections and demonstrated learning active through the task, with the current instruction taking precedence. The skill does not persist context beyond the current task or claim that current-task learning will be available later.

When the skill teaches, builds intuition, or onboards a person into a capability, model that progression explicitly. Submit the learning target, starting address, prerequisite boundary, intuition bridge, learner language, and practice or transfer evidence to Lumen. Teaching begins only when Lumen returns an executable learning proof.
...
5. run the shared version-bound acceptance loop: Lumen first, a fresh independent reader, Lumen again with reader evidence, then a fresh final reader on the identical artifact;
...
## Keep The Core Replaceable

Generated instructions, bundled runtime, shipped references, manifests, and routing inventory form the replaceable managed core. Keep credentials, organization settings, consent records, and user-created references outside that tree so an upgrade cannot overwrite them. A skill may use prior context supplied by the host for the current run, but it does not own, read, write, or migrate the host's context system. User-created skills remain user-owned and may be edited normally.

Keep remote access equally explicit. A portable runtime should expose one public HTTPS origin for its service boundary. Let the host's standard permission flow handle the first request. If access is denied, return a recoverable permission state naming only the required host. Prefer the host's native permission action so the user can approve that host directly; use one short fallback instruction only when the host cannot surface an actionable permission dialog. Retry the same operation once after approval. Do not hide a blocked connection behind a degraded cloud status, ask for unrestricted execution, or change the user's global sandbox settings.
...
4. register only skills explicitly accepted as managed product skills, then publish the update to the distribution source while preserving user-owned references;
