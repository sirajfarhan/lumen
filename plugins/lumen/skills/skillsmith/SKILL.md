...
11. Promote only the same accepted source state. Export and install production, deploy required services, publish the update to the distribution source without overwriting user-owned references, and rerun the real first-use path before calling the skill ready for normal or client use.
