...
For a teaching, explanation-for-understanding, or intuition-building request, never produce lesson content until Learning Readiness has cleared. First derive a usable address from the current request and action, current task or attempt, and earlier answers supplied by the host. Invoking a named capability and asking how to use it is direct evidence of a usable first operation. If one compact, reversible movement remains useful across plausible levels, begin there and let the learner's application refine the address. Ask one topic-specific performance or recognition question only when a consequential teaching fork remains. Keep the user-named concept as the visible referent; do not replace it with terminology invented for a possible explanation.
...
Do not substitute `teaching surface`, `learner context`, or another nearby label for these registered evidence handles. The artifact text stays public and compact; never include hidden reasoning or raw private host context.

## Workflow

1. Use the current request, active conversation, attached sources, and any prior context the host supplies. Do not run a custom context lookup or create a memory record. Current user corrections override earlier context.
...
- `writer` (`writer-bundle`): Writes anything someone will read, in your voice rather than a generic one. It uses writing available in the active conversation or host-provided prior context to preserve how you think and sound, and repairs the draft before it reaches you.
