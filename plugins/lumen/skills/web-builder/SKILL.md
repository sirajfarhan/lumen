...
9. Include source-adaptation evidence, Writer-shaped visible-copy evidence, and operational-surface evidence when those lanes shaped the artifact. When visible copy needs Writer, load the `writer` sibling skill as described in Sibling Skill Loading rather than approximating its wording. Before the first Lumen pass, run the project's build, typecheck, lint, and relevant tests when available; inspect the live artifact at desktop and around 390px mobile with screenshots; exercise required states and actions. Repair the earliest failing route, implementation, rendered-inspection, accessibility, performance, or source-transfer gate. A prose inspection note cannot replace a screenshot or direct artifact check when the environment supports inspection.
10. Run the exact rendered candidate through Lumen first and repair until it clears. Then give a fresh cold user the Lumen-cleared desktop and mobile experience plus only the intended task. Pass requires completing the primary path and required states without hidden explanation, clipped or missing content, ambiguous action, broken interaction, or unlocatable proof. Any repair restarts with Lumen. After the cold user passes, send the unchanged artifact to Lumen again with that evidence, then run a fresh final cold user on the exact later-Lumen-cleared version. Claim readiness only when both accept the identical artifact hash.
11. Repair from the returned feedback and resubmit when the model, rendered artifact, evidence, visible copy, data, or interaction changes.
