...
3. After readiness clears, read `references/concept-visualizer-model.md`, `references/evidence-model.md`, and `references/validation-model.md`. For a durable visual, begin with `references/concept-model-template.json` and fill the active evidence lanes, concept function, story reach and articulation, carrier alternatives, carrier law, target-law boundary, visual compression, prompt contract, and independent inspection evidence. Treat an initial creation and a revision differently. Once the user approves any property, preserve one authoritative baseline and compile later feedback into mutate, protect, forbid, and open sets with observable success and regression tests. Scope every reference to the properties it may and must not influence, assign production authority property by property, and select the least destructive operation. A revision is not ready until candidate and baseline were compared at presentation scale and independent inspection supports admission. Keep the fixed `tangible compressed visual story glyph` discipline while letting the selected carrier determine the presentation dialect. Use the lightest run profile that can support the requested consequence. Resubmit to Lumen whenever the prompt, generated artifact, baseline, change set, or inspection evidence changes, because a previous result goes stale the moment the artifact does.
4. Build the concept model, visual story choice, prompt or image artifact, and public inspection evidence. When the user requests an image and image generation is available, generate the actual image and inspect it directly; the requested artifact is the image itself, with the prompt as its record.
...
6. Run the packet through Lumen before claiming prompt or image readiness.
7. Repair from the returned feedback and resubmit when the model, prompt, or artifact changes.
