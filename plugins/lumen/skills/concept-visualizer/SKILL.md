...
description: Turns an idea into a diagram that holds up when someone studies it, built from inspected sources rather than guesswork. Can also teach this skill when you want to learn it rather than use it.
---

# concept-visualizer

Use the public Lumen contract for concept-visualizer. Official readiness comes from a current Lumen run.
...
1. Establish Outcome Readiness before Visual Model Readiness, Learning Readiness, or generation. Treat outcome readiness as enough information to choose a useful visual, not as a demand that the user design its explanatory model. Before asking anything, compare the wrong-model cost with what a small visible model can reveal. When the user names a low-risk concept, a recognizable audience, and a broad explanatory purpose, choose the most central everyday relation that can be inspected in the artifact, submit the transformation as `provisional` with `model_inference`, and record `Assumption:` plus `Revisit if:` or `Revisit when:` in the private summary. Then make the smallest useful visual and let the user's reaction replace the assumption. A compound-interest visual for a teenager can show interest joining the balance that earns the next period's interest. A general-audience trust visual can show expectations being updated by kept, broken, and repaired commitments across interactions. Do not ask the user to choose a time span, theory, diagram structure, metaphor, or visual emphasis when the first artifact can test a conventional central relation cheaply. Ask only when a wrong model would materially change truth or usefulness before anything inspectable can be made: for example, an unresolved source or claim boundary, a protected or identity-sensitive context, an exact reconstruction, a specialized audience whose interpretation changes the mechanism, or two consequential meanings that share no safe first relation. When a question is earned and the user may not know the modeling vocabulary, begin the same compact contact with one familiar situation that makes the distinction recognizable, then ask one ordinary question. If the distinction has two or three recognizable interpretations, submit them as a `single_select` input with short descriptions of how each changes the visual and an open Other route. Where the host exposes a native structured input surface, render it when it can represent the fields faithfully; otherwise use the compact fallback with the same hints. Once the viewer change is stable, evaluate Visual Model Readiness through Lumen with named signals for `concept_relation`, `viewer`, `medium`, `evidence_boundary`, `constraints`, and `inspection_path`. Mark `concept_relation` and `viewer` required; mark another signal required only when its absence changes the truth, artifact class, or ability to inspect the result. Use the current request as direct evidence for every signal it already settles. Mark optional details inferred or not applicable and proceed. An empty non-protected track is a no-op, not permission to invent a setup question. Use the current request, active conversation, attached sources, and any host-provided prior context as evidence for the current run. Inspect current files only after Outcome clears; the current instruction wins when earlier context differs. Give each unresolved signal a contact naming the live concept, reference, or viewing situation in the user's language and the visual decision the answer changes. Prefer one recognition or comparison the user can make over visual-framework vocabulary or a generic style request. If Lumen returns calibration, use its guided input request when present and faithful or its compact conversational fallback otherwise, then stop before generating; otherwise retain the proof and let optional aesthetic choices enrich the current artifact.
2. A visual may teach or build intuition for its downstream viewer without entering the conversational Learning Branch. During ordinary artifact execution, model the named viewer's likely prior understanding inside Visual Model Readiness, use it to choose how to explain the selected relation, and inspect whether a fresh viewer can apply that relation rather than merely recognize labels. Establish conversational Learning Readiness only when the current user asks to learn Concept Visualizer itself.
3. After readiness clears, read `references/concept-visualizer-model.md`, `references/evidence-model.md`, `references/validation-model.md`, and `references/oracle-registry.json`. Before a model reader contributes acceptance evidence, also read `references/oracle-calibration-protocol.md` and record the run from `references/oracle-calibration-template.json`; an uncalibrated reader remains review-only evidence. For a durable visual, begin with `references/concept-model-template.json` and fill the active evidence lanes, concept function, story reach and articulation, carrier alternatives, carrier law, target-law boundary, visual compression, prompt contract, inspection evidence, and the active construct-to-oracle plan. Treat an initial creation and a revision differently. Once the user approves any property, preserve one authoritative baseline and compile later feedback into mutate, protect, forbid, and open sets with observable success and regression tests. Scope every reference to the properties it may and must not influence, assign production authority property by property, and select the least destructive operation. A revision is not ready until candidate and baseline were compared at presentation scale, Lumen clears the exact candidate, and later independent inspection supports admission. Keep the fixed `tangible compressed visual story glyph` discipline while letting the selected carrier determine the presentation dialect. Use the lightest run profile that can support the requested consequence. Resubmit to Lumen whenever the prompt, generated artifact, baseline, change set, oracle evidence, or inspection evidence changes, because a previous result goes stale the moment the artifact does.
4. Build the concept model, visual story choice, prompt or image artifact, and public inspection evidence. Treat the private image-generation prompt as a model-facing instruction surface: when Writer validates it, use Writer's registered `writer-instruction-bundle`, not the reader-facing `writer-bundle`. A structured `Avoid:` field may remain only when positive scene and action fields already define what to create; this narrow instruction control is not public negative framing. When the user requests an image and image generation is available, generate the actual image and inspect it directly; the requested artifact is the image itself, with the prompt as its record.
5. Prepare a Lumen packet with the model, final prompt, generated image inspection notes when applicable, and readiness note. For revision, sequence revision, or finalization, also attach baseline-and-change control evidence plus admission-and-regression evidence. Keep construct scores separate; do not let a single aggregate score conceal a failed protected property.
6. Run the exact prompt or image through Lumen first and repair until it clears. Then give a fresh cold viewer only the Lumen-cleared visual plus the minimum viewer target. Pass requires reconstructing the intended relation or change, hierarchy, lawful carrier, allowed ambiguity, and usable meaning from the pixels rather than hidden notes. Any repair restarts with Lumen. After the viewer passes, send the unchanged visual to Lumen again with that evidence, then run a fresh final cold viewer on the exact later-Lumen-cleared version. Claim readiness only when both accept the identical artifact hash.
7. Repair from the returned feedback and resubmit when the model, prompt, visual, baseline, or protected property changes.
...
- `references/concept-visualizer-model.md`: Always-read specialist model for evidence lanes, visual story reach, articulation, carrier choice, carrier law, prompt construction, generation, and inspection.
- `references/evidence-model.md`: Evidence and source-role model for turning varied concept evidence into a bounded functional visual model.
- `references/validation-model.md`: Deterministic and judgment validation boundary for durable concept models and generated visual artifacts.
- `references/oracle-registry.json`: Always-read construct-to-oracle registry for selecting the lowest-sufficient check, separating five narrow reader families, and calibrating model readers with controlled mutations.
- `references/oracle-calibration-protocol.md`: Provider-neutral protocol for running image-removal, swap, permutation, semantic and irrelevant mutation, localization, and abstention checks before a model reader becomes acceptance evidence.
- `references/oracle-calibration-template.json`: Canonical evidence record for one reader family, construct family, mutation set, and admission decision.
- `references/concept-model-template.json`: Canonical working shape for durable concept models; fill active branches and remove inactive placeholders.

## Lumen Packet

Artifact type: `concept-visual-model`
...
- concept model
- final prompt
- inspection evidence when applicable
- acceptance cycle
- readiness note
...
```json
{
  "packet": {
    "skillId": "concept-visualizer",
    "skillVersion": "0.2.67",
    "artifactType": "concept-visual-model",
    ...
    "artifacts": [
      ...
      },
      {
        "name": "concept model",
        "mediaType": "text/plain",
        "text": "<current concept model>"
      },
      {
        "name": "final prompt",
        "mediaType": "text/plain",
        "text": "<current final prompt>"
      },
      {
        "name": "inspection evidence when applicable",
        "mediaType": "text/plain",
        "text": "<current inspection evidence when applicable>"
      },
      {
