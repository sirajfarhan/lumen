...
Read `references/evidence-model.md` and `references/oracle-registry.json` before substantial work. Use `references/concept-model-template.json` as the working shape for the model. Save a filled model when the user wants more than a quick prompt or when an image will be generated.
...
## Match Each Claim To An Oracle

Do not give one cold reader the whole theory stack. Before inspection, open `references/oracle-registry.json` and select only the constructs whose failure could change admission. For each selected construct, record the claim, observable evidence, primary oracle, controlled defect, and stopping boundary. The oracle may claim no more than its evidence supports.

Use direct structure only for facts that are actually available as structure: editable coordinates, distances, overlaps, margins, font metadata, contrast, identifiers, hashes, declared channel reuse, and citation presence. On a flattened raster, OCR boxes, saliency maps, edge maps, similarity measures, and spacing estimates become warnings or localization aids. They do not become hard truth about hierarchy, meaning, story, harmony, or transfer.

Split judgment-heavy inspection into narrow independent passes. The perceptual-structure reader inspects hierarchy, grouping, alignment, spacing, collision, legibility, and competing detail. The meaning reader reconstructs one movement, channel meaning, story reach, the nearest wrong story, and the usable landing. The world-law reader tests carrier fit, causal law, physical plausibility, and spatial reachability. The evidence reader compares the visual with its source and the explicit must-survive and may-change contract. The continuity reader compares baseline or sequence identity with the declared delta and protected properties. Do not combine these into one eloquent omnibus review.

A model reader becomes acceptance evidence only after its reader family passes the calibration pack in the registry. Read `references/oracle-calibration-protocol.md`, start from `references/oracle-calibration-template.json`, and bind the record to the reader family, construct ids, artifact class, task wording, candidate hashes, and model identity when known. First verify that every neutral surface actually rendered through the host image path; a blank pixel-identical control or wrong dimension invalidates the harness instead of failing the reader. Then remove the image and require a weaker answer or abstention. Swap the image and require the localized finding to change. Reverse candidate order and require a pairwise preference to remain stable. Seed a story-bearing defect and require detection. Apply an irrelevant mutation and require stability. Require every finding to name the visible region or relation that supports it. Let the reader answer `insufficient evidence`. A named check with `pending` evidence does not calibrate the reader.

For subjective comparison, compare candidate against baseline or one alternative on one construct at a time. Reverse the order. Keep the preference only when it survives reversal with the same localized reason. A scalar model score is a diagnostic description, not admission.

The final cold reader remains separate. It sees only the exact final surface, the minimum audience description, and the realistic viewing task. It does not see the prompt, semantic-channel ledger, source cards, creator rationale, earlier critique, or expected answer. It can establish reconstruction and prospective understanding only for that simulated reader. Claims about real audience comprehension, transfer, room-scale legibility, culturally loaded semantics, or taste still require a real audience task.
...
Then run the semantic-attractor veto. Some carriers do not merely suggest a neighboring concept; their ordinary function already gives the viewer a stronger answer. When that happens, treat the carrier as a poor fit until the target reading clearly dominates. Keep it only when the target cue is larger, simpler, and easier to see than the carrier's native function. If the viewer can name the carrier's usual job before seeing the target relation, and the target depends on a small trace or private explanation, replace the carrier.
...
For behavior validation or acceptance panels, first repair the exact generated visual through Lumen until it clears, then run a separate blind reconstruction harness. The creator prepares the validation target and the evidence bundle, then stops. A separate evaluator sees neutral copies of that Lumen-cleared image only, with no process trail to lean on. Ask the evaluator to state that image-only boundary before scoring. The score should test whether the pixels recover the concept as a lawful story rather than a label. The evaluator should see material evidence become object roles, object roles obey a world law, and that world law carry the chosen level. A sequence adds ordered change inside the same world. Store the result in an external validation report. Point to that report from the model only when the model is being used as an after-the-fact acceptance wrapper. During creator builds, leave blind-result fields blank or `performed: false`. Recovery optimization cannot overrule the other gates: a target name is not enough if the story, world law, or grounded reality fails. Any edit restarts with Lumen. After a blind pass, resubmit the unchanged visual to Lumen with the reader evidence, then run a fresh final blind reconstruction on the exact later-Lumen-cleared version.

Preserve prompt-label separation. The final prompt draft should keep the contract as separate fields rather than one long paragraph. Validate this private generator prompt through Writer's `writer-instruction-bundle`, because it is a model-facing instruction surface rather than reader-facing prose. A structured `Avoid:` field is valid when positive scene and action fields already govern what to create; do not let that narrow control become a general exemption for negative-first public writing. If writer returns `PASS WITH REVIEW`, inspect the warning. In zero-caveat mode, repair the prompt or validation artifact until writer returns a clean `PASS`.

## Validate And Return

Before trusting the result, fill and save the model JSON whenever the run will generate an image or count as validation evidence. A prompt-only quick sketch can be compact, but behavior validation must have a full model file. In an installed run, submit that model through the bundled Lumen client before treating the result as evidence. During skill development, also run the canonical source-tree validator and mutation harness; those release checks do not replace Lumen or independent inspection.
