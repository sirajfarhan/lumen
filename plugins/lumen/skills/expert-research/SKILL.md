...
description: Researches a decision where being wrong gets expensive. Every claim traces back to a source that was actually read, so you can check the reasoning instead of trusting it. Can also teach this skill when you want to learn it rather than use it.
---

# expert-research

Use the public Lumen contract for expert-research. Official readiness comes from a current Lumen run.
...
1. Establish Outcome Readiness before Decision Readiness or research. Treat an explicit request to research whether, should, compare, choose, buy, hire, adopt, or use something as direct evidence of the decision the research must support; a stated time horizon such as `this year` is direct scope evidence. Do not ask what that decision should make possible when deciding is already the stated job. Once the requested decision or learning outcome is stable, evaluate Decision Readiness through Lumen using its scope, consequence, recency and evidence standard, and real constraints. Use the current request, active conversation, attached sources, and any host-provided prior context first; the current instruction wins. Interrupt only when a response changes the source bar, claim boundary, comparison, or handoff enough to earn the burden. For a low-risk general decision, proceed with a bounded central case and make volume, budget, workflow, or integration assumptions visible in the report. Represent each reversible working assumption as `provisional` with `model_inference`, a private `Assumption:` statement, and a `Revisit if:` or `Revisit when:` condition; do not leave it merely inferred when it is strong enough to support a bounded scenario. Ask only when one missing condition could reverse the recommendation and cannot be handled through scenarios or a pilot. Do not block general research on an optional source: answer the defensible general question and offer a source-specific audit afterward. When a source-specific audit is the requested job and its exact source is absent, represent the gap as a typed `source_request`, not a clarification question. Put the source in `contact.focus`, the inspection in `contact.use`, and only a result-changing original-form requirement in `contact.fidelity`; Lumen builds the visible request. Never ask the user to describe evidence the research can inspect. For a genuinely unstated decision, use a `question` contact with a live focus and concrete use. Start with the capability gap only when the user named a candidate without saying what choice the research should support: for a fractional CFO topic with no decision, ask `What are you hoping a fractional CFO would help you handle that your current setup cannot?` Prefer the nearest real consequence over a generic request for scope. Choose risk from the research operation rather than the subject alone. Public-evidence research about healthcare, legal, financial, safety, or privacy concerns stays adaptive when it maps a reversible decision without private data or authority; preserve those concerns as report conditions. Use protected risk only when the research itself depends on regulated data, actual consent or authority, required private sources, safety clearance, or an irreversible action. If Lumen returns calibration, use its staged input request when the host can render it faithfully and its conversational fallback otherwise, then stop without process commentary; otherwise proceed with the narrowest defensible interpretation and retain the current proof.
2. When the request is meant to teach or build intuition rather than only hand off a decision, establish Learning Readiness separately, update it after each learner response, and require fresh retrieval or transfer before calling the learning cycle verified.
3. After readiness clears, read `references/expert-research-model.md`, `references/method.md`, and the machine-readable `references/evidence-lineage-template.json` for substantial work. Shape one decision under consequence and design source variance before searching. Keep search results, graphs, APIs, model suggestions, and associations as typed discovery leads. Only an inspected source-local anchor from a qualified current source may create SUPPORTS, OPPOSES, or QUALIFIES evidence for one atomic claim. Track dependence beyond citation echo, preserve dissent and limits, and stop when another source would no longer change the decision.
4. Create the least-sufficient research evidence map beside every durable report. Preserve qualified versioned sources, inspected anchors, atomic claims, typed evidence links, explicit lead/dependency/derivation/update lists, a reproducible trajectory, core stage results, and the decision handoff. Activate transfer contracts, coverage maps, and legal, multilingual, multimodal, executable, forensic, private-corpus, forecasting, or active-acquisition adapters only when they can change the decision. Cross-domain relations may transfer only with separate target-domain evidence; source-domain support never copies. Corrections and retractions must mark dependent claims for review before recomputation.
5. Shape the reader-facing report only after the evidence boundary is stable. When careful wording needs Writer, load the `writer` sibling skill as described in Sibling Skill Loading and apply it in the same work phase; do not imitate it from memory. A progress update at this transition should describe what the report is becoming, such as `I am turning the evidence into a compact owner-facing decision memo`, rather than narrating internal steps — that is about usefulness, not concealment, so name the skills involved plainly if the user asks. After the final writing pass, bind the evidence map to the exact trimmed report and Repair the earliest failed research stage rather than polishing around it.
6. Prepare a Lumen packet with the exact research report, structured research evidence map, decision handoff, clean local report-plus-lineage validation output, and risk or repair summary.
7. Run the exact report through Lumen first and repair until it clears the research gates. Then give a fresh cold decision reader only the Lumen-cleared report and the minimum decision context. Pass requires reconstructing the decision, the conditions on each major claim, confidence boundaries, meaningful dissent or limits, and the next action. Any repair restarts with Lumen. After the reader passes, send the unchanged report to Lumen again with that evidence, then run a fresh final cold decision reader on the exact later-Lumen-cleared version. Claim decision readiness only when both accept the identical artifact hash.
8. Repair from the returned feedback and resubmit when the report, evidence map, citations, confidence, or handoff changes.
...
- Consider running `question-sharpener` first only when the decision itself is unformed or keeps shifting — not merely under-detailed — otherwise work directly. When you do run it, load it as described in Sibling Skill Loading, follow its `sharpened-question` as the structure this work takes, and record the handoff in the packet's composition record. When you work solo, record in one line why the direct approach was right.
...
- `references/expert-research-model.md`: Always-read specialist model for decision shaping, earned-source selection, conditional claims, convergence, synthesis, and cold-reader validation.
- `references/method.md`: Detailed evidence method for source qualification, claim extraction, association tools, dependency-aware consensus, transfer, coverage, updates, conditional adapters, and decision handoff.
- `references/evidence-lineage-template.json`: Machine-readable provenance contract for sources, anchors, leads, claims, evidence links, dependencies, derivations, updates, trajectory, stage diagnosis, transfer, coverage, and decision binding.
...
- research report
- structured research evidence map
- decision handoff
- acceptance cycle
- risk or repair summary
  ...
  "packet": {
    ...
    "artifacts": [
      ...
      },
      {
        ...
        "mediaType": "text/plain",
        "text": "<current research report>"
      },
      {
        "name": "structured research evidence map",
        "mediaType": "text/plain",
        "text": "<current structured research evidence map>"
      },
      {
        "name": "decision handoff",
        "mediaType": "text/plain",
        "text": "<current decision handoff>"
      },
      {
      ...
      {
        "name": "risk or repair summary",
        "mediaType": "text/plain",
        "text": "<current risk or repair summary>"
      }
    ],
    "contextRefs": [],
    "publicTrace": {},
